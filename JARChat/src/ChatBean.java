...
public class ChatBean {
	...
	@Path("/post/{text}/{user}")
	@Produces(MediaType.TEXT_PLAIN)
	public String post(@PathParam("text") String text, @PathParam("user") String user) {
	...
	public String register(String info) {
		...
		char[] unamechar = uname.toCharArray();
		boolean validname = true;
				
		for(char c: unamechar) {
	...
	public String login(String info) {
		...
		try(InputStream in = Files.newInputStream(p);
			...
			String line = null;
			while((line = reader.readLine()) != null) {
				if(uname.equals(line.split(":")[0]) && pw.equals(line.split(":")[1])) {
					found = true;
					break;
				}
			}
